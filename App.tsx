...
const App: React.FC = () => {
  ...
  // 이미지 높이 설정: 4, 5페이지 이미지 크기를 다른 페이지 수준(33%)으로 키움
  ...
  return (
    <div className="flex justify-center items-center w-full h-[100dvh] bg-gray-200 font-sans overflow-hidden p-3 sm:p-0">
      <div 
        className="relative w-full max-w-[390px] h-[92dvh] sm:h-[82vh] sm:rounded-[32px] bg-white shadow-xl overflow-hidden flex flex-col select-none border border-white/20"
      ...
      >
        ...
        {/* 2. 텍스트 컨텐츠 영역 */}
        <div className={`flex-1 flex flex-col px-6 ${currentIndex >= 3 ? 'pt-12 pb-2' : 'pt-8 pb-3'} bg-white min-h-0 overflow-hidden transition-all`}>
          ...
          {/* 본문 내용: 4, 5페이지는 간격을 넓혀 아래로 여유있게 배치 */}
          <div className={`flex-1 flex flex-col no-scrollbar ${currentIndex >= 3 ? 'space-y-5' : 'space-y-2.5'}`}>
            ...
            {/* 4페이지 배지: 텍스트 바로 아래로 바짝 붙여 위로 올림 */}
            ...
            {/* 해시태그 영역 (하단): 첫 번째 카드일 때만 본문 아래에 표시 */}
            {currentCard.keyword && currentIndex === 0 && (
          ...
          {/* 하단 버튼 영역: 기부 버튼 */}
          {isLastPage && (
            <div className="mt-2 mb-2 flex-shrink-0 flex flex-col items-center">
              <button 
                onClick={() => window.open(CARD_NEWS_CONFIG.links.donation, '_blank')}
                className="w-[85%] bg-emerald-500 text-white py-2.5 rounded-xl font-bold text-[15px] flex items-center justify-center gap-2 shadow-lg active:scale-95 transition-transform"
              >
        ...
        <div className="bg-gray-50 px-4 py-2 border-t border-gray-100 flex-shrink-0">
          ...
          {isLastPage && (
            <div className="grid grid-cols-2 gap-2 mb-2">
               <button onClick={() => window.open(CARD_NEWS_CONFIG.links.homepage, '_blank')} className="bg-white py-1 rounded-lg border border-gray-200 text-[10px] font-bold text-gray-500 flex items-center justify-center gap-1 active:bg-gray-50">
                 <ExternalLink size={10} /> 홈페이지
               </button>
               <button onClick={() => window.open(CARD_NEWS_CONFIG.links.taxBenefit, '_blank')} className="bg-white py-1 rounded-lg border border-gray-200 text-[10px] font-bold text-gray-500 flex items-center justify-center gap-1 active:bg-gray-50">
          ...
          <div className="flex flex-col items-center gap-0.5">
             ...
             <a 
               ...
               className="flex items-center gap-1.5 px-3 py-0.5 bg-white border border-emerald-100 rounded-full text-[10px] text-emerald-600 font-bold shadow-sm active:bg-emerald-50"
             >
